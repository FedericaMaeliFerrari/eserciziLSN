...
void GA::NewGeneration_Metropolis(double temp){
	for(int i=0;i<100;i++){					//matrice di appoggio in cui salvo i percorsi precedenti;
		for(int j=0;j<_num;j++){
			_percorsiOld[i][j]=_percorsi[i][j];
		}
	}
	for(int i=0;i<100;i++){
		_distanzaOld[i]=_distanza[i];			//vettore in cui salvo i valori della distanza precedenti
	}

	Mutazione();

	double p;						//Confronto percorso per percorso la popolazione mutata e quella precedente.
	double r;						//Se la lunghezza di un percorso risulta minore prima della mutazione allora rifiuto il cambiamento e tengo il 
	for(int i=0;i<100;i++){					//percorso vecchio nella nuova popolazione (rifiuto la mutazione)
		if(_distanza[i]>_distanzaOld[i]){
			p=exp(-1./temp*(_distanza[i]-_distanzaOld[i]));
		}
		else{ p=1.;}
		r=_rnd->Rannyu();
		if(r>p){
			for(int j=0;j<32;j++){
				_percorsi[i][j]=_percorsiOld[i][j];
			}
			_distanza[i]=_distanzaOld[i];
		}
	}
